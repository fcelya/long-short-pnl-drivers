...
for i in range(N_BATCHES):
    pairs_file = f'pkl_data/{UNIVERSE}_pairs_MLE_v9_{i}.parquet'
    ...
    if len(pairs_df):
        ...
        if RECALC_RETURNS or RECALC_PAIRS:
            ...
            df.to_parquet(f'pkl_data/results_v9_{UNIVERSE}_{i}_lean.parquet')
            print(f'Saved pkl_data/results_v9_{UNIVERSE}_{i}_lean.parquet')
            df = pd.concat(long_results_lean)
            df = df.dropna(how='all')
            df = df.groupby(df.index).agg(['sum','count'])
            df.to_parquet(f'pkl_data/results_v9_long_{UNIVERSE}_{i}_lean.parquet')
            print(f'Saved pkl_data/results_v9_long_{UNIVERSE}_{i}_lean.parquet')
            df = pd.concat(short_results_lean)
            df = df.dropna(how='all')
            df = df.groupby(df.index).agg(['sum','count'])
            df.to_parquet(f'pkl_data/results_v9_short_{UNIVERSE}_{i}_lean.parquet')
            print(f'Saved pkl_data/results_v9_short_{UNIVERSE}_{i}_lean.parquet')
    else:
...
for i in range(N_BATCHES):
    try:
        df = pd.read_parquet(f'pkl_data/results_v9_{UNIVERSE}_{i}_lean.parquet')
        dfs.append(df)
    except Exception as e:
...
returns_file = f'returns_{UNIVERSE}_v9.csv'
...
for i in range(N_BATCHES):
    try:
        df = pd.read_parquet(f'pkl_data/results_v9_long_{UNIVERSE}_{i}_lean.parquet')
        dfs.append(df)
    except Exception as e:
        print(f"No file found for {i+1}/{N_BATCHES}. {e}")
...
df = df['sum'] / df['count']
df = df.to_frame(name='Daily_Return')
returns_file = f'returns_long_{UNIVERSE}_v9.csv'
df.to_csv(returns_file, index=True) 
print(f'Saved returns to {returns_file}')
...
for i in range(N_BATCHES):
    try:
        df = pd.read_parquet(f'pkl_data/results_v9_short_{UNIVERSE}_{i}_lean.parquet')
        dfs.append(df)
    except Exception as e:
        print(f"No file found for {i+1}/{N_BATCHES}. {e}")
...
df = df['sum'] / df['count']
df = df.to_frame(name='Daily_Return')
returns_file = f'returns_short_{UNIVERSE}_v9.csv'
df.to_csv(returns_file, index=True) 
print(f'Saved returns to {returns_file}')
...
positions_file = f'active_positions_{UNIVERSE}_v9.csv'
