...
def process_pair_oos(pair_row, series01_filtered, series02_filtered, rates_filtered):
    ...
    # # Get the historic volatilities
    # sigma_y = pair_row['sigma_y']*(252**.5)
    # sigma_x = pair_row['sigma_x']*(252**.5)
    ...
    # Compute thetas
    z = np.log(price_y) - np.log(price_x)
    theta_y = ((pair_row['mu_y'] - rates_filtered/100) - pair_row['lambda_1']*z)/sigma_y
    theta_x = ((pair_row['mu_x'] - rates_filtered/100) - pair_row['lambda_2']*z)/sigma_x
    ...
    # POSITION SIZING 2
    corr = pair_row['rho_xy']
    phi_y = -(delta_y - delta_x)/(gamma_y-gamma_x)*(theta_y - corr*theta_x)/(sigma_y*price_y*(1-corr**2)) - (gamma_y - price_x/price_y*gamma_x)/(gamma_y-gamma_x)
    phi_x = -(delta_x - delta_y)/(gamma_x-gamma_y)*(theta_x - corr*theta_y)/(sigma_x*price_x*(1-corr**2)) - (gamma_x - price_y/price_x*gamma_y)/(gamma_x-gamma_y)

    # # TRIGGER BEST
    open_val = (1 - pair_row["lambda_1"] - pair_row["lambda_2"]) * sigmaRN_z
    trigger = np.where(z_spread > open_val, 1, np.nan)

    # if len(phi_y[phi_y>4])>0:
    #     print(f"{pair_row['Stock_y']} - {pair_row['Stock_x']}")

    # Check if all triggers are NaN
    if not np.isnan(trigger).all():
        # Calculate the pair returns
        returns, long_returns, short_returns = calculate_returns(price_y, price_x, rates_filtered/100/YEAR, phi_y, phi_x, trigger)
...
df_original = pd.read_parquet(f'Pairs_SP500_FPT/{UNIVERSE}_impl_vols_clean.parquet')
...
# df_original = df_original[df_original['Date']>='2009-01-01']    # filter the df so it only mantains the desired dates (we have to take one day prior to the first to do the estimations)
...
if UNIVERSE == 'spx':
    ...
    # for i in range(1,3):
    #     with open(f'pkl_data/exclude_stocks_list_{i}.pkl', 'rb') as f:
    #         l = pickle.load(f)
    #     for s in l:
    #         try:
    #             series01_orig = series01_orig.drop(s, axis=1)
    #         except Exception as e:
    #             print(f'{s} not found in df for dropping. {e}')
...
for i in range(N_BATCHES):
    pairs_file = f'pkl_data/{UNIVERSE}_pairs_MLE_v10_{i}.parquet'
    ...
    if len(pairs_df):
        ...
        if RECALC_RETURNS or RECALC_PAIRS:
            ...
            df.to_parquet(f'pkl_data/results_v10_{UNIVERSE}_{i}_lean.parquet')
            print(f'Saved pkl_data/results_v10_{UNIVERSE}_{i}_lean.parquet')
            df = pd.concat(long_results_lean)
            df = df.dropna(how='all')
            df = df.groupby(df.index).agg(['sum','count'])
            df.to_parquet(f'pkl_data/results_v10_long_{UNIVERSE}_{i}_lean.parquet')
            print(f'Saved pkl_data/results_v10_long_{UNIVERSE}_{i}_lean.parquet')
            df = pd.concat(short_results_lean)
            df = df.dropna(how='all')
            df = df.groupby(df.index).agg(['sum','count'])
            df.to_parquet(f'pkl_data/results_v10_short_{UNIVERSE}_{i}_lean.parquet')
            print(f'Saved pkl_data/results_v10_short_{UNIVERSE}_{i}_lean.parquet')
    else:
...
for i in range(N_BATCHES):
    try:
        df = pd.read_parquet(f'pkl_data/results_v10_{UNIVERSE}_{i}_lean.parquet')
        dfs.append(df)
    except Exception as e:
...
returns_file = f'returns_{UNIVERSE}_v10.csv'
...
for i in range(N_BATCHES):
    try:
        df = pd.read_parquet(f'pkl_data/results_v10_long_{UNIVERSE}_{i}_lean.parquet')
        dfs.append(df)
    except Exception as e:
        print(f"No file found for {i+1}/{N_BATCHES}. {e}")
...
df = df['sum'] / df['count']
df = df.to_frame(name='Daily_Return')
returns_file = f'returns_long_{UNIVERSE}_v10.csv'
df.to_csv(returns_file, index=True) 
print(f'Saved returns to {returns_file}')
...
for i in range(N_BATCHES):
    try:
        df = pd.read_parquet(f'pkl_data/results_v10_short_{UNIVERSE}_{i}_lean.parquet')
        dfs.append(df)
    except Exception as e:
        print(f"No file found for {i+1}/{N_BATCHES}. {e}")
...
df = df['sum'] / df['count']
df = df.to_frame(name='Daily_Return')
returns_file = f'returns_short_{UNIVERSE}_v10.csv'
df.to_csv(returns_file, index=True) 
print(f'Saved returns to {returns_file}')
...
positions_file = f'active_positions_{UNIVERSE}_v10.csv'
